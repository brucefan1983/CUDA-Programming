...
(real *g_x, real *g_inner, int N)
{
    ...
    {
        tmp_sum = s_inner[tid];
        tmp_sum += s_inner[tid + 32]; __syncwarp();
        s_inner[tid] = tmp_sum;       __syncwarp();
        tmp_sum += s_inner[tid + 16]; __syncwarp();
        s_inner[tid] = tmp_sum;       __syncwarp();
        tmp_sum += s_inner[tid + 8];  __syncwarp();
        s_inner[tid] = tmp_sum;       __syncwarp();
        tmp_sum += s_inner[tid + 4];  __syncwarp();
        s_inner[tid] = tmp_sum;       __syncwarp();
        tmp_sum += s_inner[tid + 2];  __syncwarp();
        s_inner[tid] = tmp_sum;       __syncwarp();
        tmp_sum += s_inner[tid + 1];  __syncwarp();
    }

    if (tid == 0)
    {
        atomicAdd(g_inner, tmp_sum);
    }
}
