...
)
{
    ...
    if (i < N)
    {
        ...
        int NN = g_NN[i];
#ifdef USE_LDG
        real x_i = __ldg(g_x + i);
        real y_i = __ldg(g_y + i);
        real z_i = __ldg(g_z + i);
#else
        real x_i = g_x[i];
        real y_i = g_y[i];
        real z_i = g_z[i];
        ...
        {   
            int j = g_NL[i + N * k];
#ifdef USE_LDG
            real x_ij  = __ldg(g_x + j) - x_i;
            real y_ij  = __ldg(g_y + j) - y_i;
            real z_ij  = __ldg(g_z + j) - z_i;
#else
            real x_ij  = g_x[j] - x_i;
            real y_ij  = g_y[j] - y_i;
            real z_ij  = g_z[j] - z_i;
...
void find_force(int N, int MN, Atom *atom)
{
    ...
    (
        lj, N,  atom->NN, atom->NL, box,
        atom->x, atom->y, atom->z,
        atom->fx, atom->fy, atom->fz, atom->pe
    );
}
